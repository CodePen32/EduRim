...
func (r *SubscriptionRepository) GetAllUserSubscriptions() ([]models.UserSubscription, error) {
	if r.db == nil {
		return nil, ErrNoDB
	}
	rows, err := r.db.Query(`
...
func (r *SubscriptionRepository) GetAllRequests(status string) ([]models.SubscriptionRequest, error) {
	if r.db == nil {
		return nil, ErrNoDB
	}
	...
	if bacBranchID.Valid {
		rows, err = r.db.Query(
			`SELECT id, name, COALESCE(description,''), duration_days, price, learning_path_id, bac_branch_id, is_active, COALESCE(created_at,NOW())
			 FROM subscription_plans WHERE is_active=1 AND learning_path_id=? AND bac_branch_id=?`,
			learningPathID, bacBranchID.Int64,
		)
	} else {
		rows, err = r.db.Query(
			`SELECT id, name, COALESCE(description,''), duration_days, price, learning_path_id, bac_branch_id, is_active, COALESCE(created_at,NOW())
			 FROM subscription_plans WHERE is_active=1 AND learning_path_id=? AND (bac_branch_id IS NULL OR bac_branch_id=0)`,
			learningPathID,
		)
	}
	if err != nil {
