...
export function DashboardPage() {
  ...
  return (
    <div>
      {/* Page header */}
      <div style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', padding: '12px 16px', marginBottom: 20 }}>
        <p style={{ fontSize: 16, fontWeight: 700, color: '#1E293B', fontFamily: 'Cairo', margin: 0 }}>لوحة القسم</p>
        {scope && <p style={{ fontSize: 12, color: '#64748B', fontFamily: 'Cairo', marginTop: 2, margin: 0 }}>نظرة عامة — {scope.label}</p>}
      </div>

      {loading ? <Loading /> : error ? <ErrorState message={error} onRetry={load} /> : stats && (
        <div className="dashboard-cards-grid" style={{ padding: '0 16px 24px' }}>
            ...
            return (
              ...
              >
                <div style={{ width: 44, height: 44, borderRadius: 12, background: iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill={iconColor}>{icon}</svg>
                </div>
                <p style={{ fontSize: 30, fontWeight: 700, color: '#1E293B', fontFamily: 'Cairo', lineHeight: 1, margin: 0 }}>
                  {value}
                </p>
                <p style={{ fontSize: 13, color: '#64748B', fontFamily: 'Cairo', marginTop: 6, margin: '6px 0 0' }}>{label}</p>
              </div>
            )
          })}
        </div>
